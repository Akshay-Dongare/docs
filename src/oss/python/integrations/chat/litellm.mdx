...
- `ChatLiteLLM`: The main LangChain chat wrapper for LiteLLM ([API reference](https://reference.langchain.com/python/langchain-litellm/chat_models/litellm/ChatLiteLLM)).
- `ChatLiteLLMRouter`: A `ChatLiteLLM` wrapper that leverages LiteLLM's Router for load balancing and fallbacks ([API reference](https://reference.langchain.com/python/langchain-litellm/chat_models/litellm_router/ChatLiteLLMRouter)).

The package also ships [`LiteLLMEmbeddings`](https://reference.langchain.com/python/langchain-litellm/embeddings/litellm/LiteLLMEmbeddings), [`LiteLLMEmbeddingsRouter`](https://reference.langchain.com/python/langchain-litellm/embeddings/litellm_router/LiteLLMEmbeddingsRouter), and [`LiteLLMOCRLoader`](https://reference.langchain.com/python/langchain-litellm/document_loaders/litellm_ocr/LiteLLMOCRLoader)—see the [providers page](/oss/integrations/providers/litellm) for details.
...
Use Google Search grounding with Vertex AI models (e.g., `gemini-2.5-flash`). Citations and metadata are returned in `response_metadata` (batch) or `additional_kwargs` (streaming).
